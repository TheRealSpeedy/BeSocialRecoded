...
public class Handshake implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (Players.notMember((Player) sender)) {
			...
			if (args.length == 0) {
				sender.sendMessage(Messages.getPrefix() + ChatColor.RESET + "" + ChatColor.RED + "/handshake <playername>");
			}
			else {

				if (Cooldown.cooldownActive((Player) sender, "handshake")){
