...
public class Poke implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (Players.notMember((Player) sender)) {
			...
			if (args.length == 0) {
				sender.sendMessage(Messages.getPrefix() + ChatColor.RESET + "" + ChatColor.RED + "/poke <playername>");
			}
			else {

				if (Cooldown.cooldownActive((Player) sender, "poke")){
				...
				if (target == null) {
					...
					sender.sendMessage(Messages.getPrefix() + Messages.getInfoMessage("messages.sender.error.selfSocial.poke"));
					...
					if (Players.notMember(target)) {
						...
						sender.sendMessage(Messages.getPrefix() + Messages.getSocialMessage("messages.sender.success.poke", (Player) sender, target));
						target.sendMessage(Messages.getPrefix() + Messages.getSocialMessage("messages.target.success.poke", (Player) sender, target));
						Players.spawnParticles((Player) sender, target, cmd.getName());
					}
				}
			}
